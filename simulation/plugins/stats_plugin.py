...
class StatPlugin(IPlugin):
    ...
    def on_sim_done(self, instance):
        ...
        for lot_name in self.lot_type_cost:
            self.cost += self.lot_type_priority[lot_name] * (
                        self.lot_type_cost[lot_name] / self.lot_type_cost_nb[lot_name])
            self.avg_cycle_time[lot_name] = round(self.act_sum[lot_name] / (self.lot_type_cost_nb[lot_name] * 3600 * 24),
                                                  2)
