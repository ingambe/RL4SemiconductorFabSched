...
@ray.remote
def one_pop_iter(seed: int, run_to: int, tools_type: list, dataset: str):
    plugins = [StatPlugin()]
    ...
    if dataset == 'HVLM':
        ...
        with open("365days_LVHM.pkl", mode="rb") as file:
            ...
            for plugin in plugins:
                plugin.on_sim_init(instance)
    net = Network(12, len(tools_type))
    net.load_state_dict(torch.load(f"pretrained_{dataset}.pt"))
    net.eval()
    with torch.no_grad():
        random.seed(seed)
...
def main():
    ...
    p.add_argument("--dataset", type=str, default="LVHM")
    p.add_argument("--days", type=int, default=365)
    a = p.parse_args()
    wandb.init(project="benchmark-fab-RL", tags=[a.dataset], group="rl_dispatching")
    ...
    tools_type.sort()
    wandb.save(f"pretrained_{a.dataset}.pt")
    output_iter = ray.get(
        [
            one_pop_iter.remote(k, run_to, tools_type, a.dataset)
    ...
    print(f'ES with {nb_workers} runs')
